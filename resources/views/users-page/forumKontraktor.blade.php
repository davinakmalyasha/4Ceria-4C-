...
<div class="container">
    ...
    <div class="project-card">
        ...
        @php
            $sudahBid = $project->bidsKontraktor->where('kontraktor_id', auth()->user()->kontraktor->id)->isNotEmpty();
        @endphp

        @if($sudahBid)
            <button class="btn-primary" disabled style="background-color: gray;">Sudah Bid</button>
        @else
            <a href="{{ route('formulir.bid.kontraktor', $project->id) }}" class="btn-primary">Ajukan Bid</a>
        @endif

        @if($project->bidsKontraktor->isNotEmpty())
                            ...
                            @if($project->user->phoneNumber->count())
                                @foreach($project->user->phoneNumber as $phone)
                                    @php
                                        ...
                                        $pesan = "Halo, saya kontraktor yang Anda pilih di proyek '" . $project->title . "'";
