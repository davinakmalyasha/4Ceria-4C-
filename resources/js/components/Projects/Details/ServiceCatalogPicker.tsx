                        ...
                        >
                            <div className="flex justify-between items-start relative z-10">
                                <div className="space-y-1 pr-8">
                                    ...
                                    <p className={`text-[13px] font-bold ${active ? 'text-slate-100' : 'text-slate-600'}`}>
                                        Rp {(Number(service.price) || 0).toLocaleString('id-ID')}
                                    </p>
                                </div>
                                <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-colors ${
                                    active ? 'bg-white text-slate-900' : 'bg-gray-50 text-gray-300'
                                }`}>
                            ...
                            {service.description && (
                                <div className={`mt-3 pt-3 border-t text-[11px] font-medium leading-relaxed ${
                                    active ? 'border-white/10 text-slate-100' : 'border-gray-50 text-gray-400'
                                }`}>
                                    {service.description}
                                </div>
                            )}
